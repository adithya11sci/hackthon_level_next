...
export const VATRefundSection: React.FC<VATRefundSectionProps> = ({ refreshKey = 0 }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (!walletAddress) {
        // Generate a QR code that will trigger wallet connection
        setQrValue(`ethereum://gemetra-vat-refund/${Date.now()}`);
      } else {
        // Use the connected wallet address
        setQrValue(`ethereum://gemetra-vat-refund/${walletAddress}/${Date.now()}`);
      }
      setStep('sign');
    } catch (error) {
